...
predict.overlap.grpreg <- function(object, X, 
    ...
    if (length(dim(object$beta)) == 2) {
        ...
        if (type == 'norm') {
            beta <- coef(object=object, lambda=lambda, latent = TRUE, 
                         which=which, drop = FALSE, ...)
            if (!latent) {
                warning("Only the L2 norm of the latent coefficients can be returned! Set latent = 'TRUE' to avoid this warning message!")
            } 
    ...
    if (!missing(X) && !is.null(X)) {
        X <- expandX(X, object$indicator.mat, object$grp.vec)
    }
    if (latent) {
        warning("Only latent 'coefficients', 'vars', 'nvars' can be returned! Set latent = 'FALSE' to avoid this warning message!")
    }
    obj.new <- object
...
coef.overlap.grpreg <- function(object, lambda, latent = FALSE, 
    ...
    beta <- coef(object = obj.new, lambda = lambda, 
                 which = which, drop=FALSE, ...)        
    if (!latent) {
        beta <- gamma2beta(beta, indicator.mat = object$indicator.mat, 
